...
void Game_决定当前操作方与应对方信息(Game *game)
{
	...
	这里的问题是，被吃子的当前应对方棋子数量已经确实被修改了实参。
	当然，修改当前应对方的棋子数量并不影响红棋数量、黑棋数量。
	但在选棋之前，又先进行了决定双方信息再选棋。
	这时上一回合应对方的棋子数量又被赋值覆盖为永远没变化的红棋数量、黑棋数量。
	*/
	...
	else
	{
		...
		if (gyf_debug_mode->DoesMsgPrint)
		{		
		printf("交换棋子数量与索引数组之后（非同步交换）：%p, %p\n", game->当前操作方的棋子的索引数组, game->当前应对方的棋子的索引数组);
		Game_show(game);
		}	
	}
}
...
/*
void 固定选棋范围_获取单方可选棋索引数组(GYF_Debug_Mode *self, Game *game)
{
	short current_index;
	for (short i=0;i<game->当前操作方的棋子数量;i++)
	{
		current_index = game->当前操作方的棋子的索引数组[i];
		if (strchr(self->固定选棋范围, game->chessBoard[current_index]))
		{
			current_index
		};
}
...
{
	switch (tolower(game->chessBoard[着法->from_index]))
	{
		case 'c':
		...
		case 'm':
		着法->to_index_of_move = 马的可活动范围缩小及准备移动到的随机落点(game->chessBoard, 着法->from_coord);
		break;
		
		case 'p':
		着法->to_index_of_move = 炮的可活动范围缩小及准备移动到的随机落点(game->chessBoard, 着法->from_coord);
		break;		
		
		default:
		着法->to_index_of_move = -1;
		break;
	}
}
...
void 举棋_完全随机(Game *game, TheStep *着法)
{
	...
	// 将选到的棋子的索引转坐标
	索引转坐标(gyf_debug_mode->DoesTest?gyf_debug_mode->固定落棋范围:着法->from_index, 着法->from_coord);
	着法->from_x=着法->from_coord[0];
	着法->from_y=着法->from_coord[1];
	
	if (gyf_debug_mode->DoesMsgPrint) puts("⚡️《举棋之可落点之检查》");
	// 计算可落点以及决定落点
	TheStep_update_toIndexOfMove(game, 着法);	
	...
	char *棋子内容_中文=malloc(10);
	strcpy(棋子内容_中文, current_player?"黑":"红");
	switch (tolower(game->chessBoard[着法->from_index]))
	{
		case 'c':
		strcat(棋子内容_中文, "车");
		...
		case 'b':
		strcat(棋子内容_中文, !strcmp(棋子内容_中文, "红")?"兵":"卒");
		break;
